...
class Group(BaseGroup):
    ...
    def reckoning(self):
        ...
        for player in players:
            player.random_energy_cost = c(np.int(np.random.normal(loc=energy_cost,scale=energy_stochasticity)))
            if self.session.config['mean_regression']:
                player.random_energy_cost = c(np.int(np.random.normal(loc=r*(mu-player.participant.payoff),scale=r**2*(1-r**2)/(1-r)**2 * sigma**2)))
        ...
        for player in players: 
            ...
            player.participant.vars['virtual_cash_bonus'] = np.array(self.session.config['real_world_currency_per_point']*int(player.participant.payoff) - 
                self.session.config['penalty']*player.rounds_below_threshold*self.session.config['desperation']).clip(0)
